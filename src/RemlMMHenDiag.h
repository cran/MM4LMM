...
List MM_RemlRcppHenDiag(VectorXd & Y, MatrixXd & X, MatrixXd & Z , List & GList , List & GinvList , VectorXd & Rinv , VectorXd logdetV , VectorXd & Init , int MaxIter, double Crit){
  ...
  GinvCompl.setZero();
  MatrixXd xR(X.transpose());
  for (int i = 0; i < X.rows(); i++) xR.col(i)=X.row(i)*Rinv(i);
  MatrixXd xRx = xR * X;
  MatrixXd xRx_inv(xRx);
  double detR,detRinv;
  sym_inverse(xRx,xRx_inv,detR,detRinv,0);
  VectorXd R(as<Map<VectorXd> >(GList[NbVar-1]));
  xRx = xR*X;
  VectorXd DimG(NbVar);
  DimG.setZero();
  ...
  MatrixXd RinvX = xR.transpose();
  MatrixXd ZRinv (Z.transpose());
  for (int i =0;i<Z.rows();i++) ZRinv.col(i) = Z.row(i)*Rinv(i);
  MatrixXd RinvZ = ZRinv.transpose();
  MatrixXd xRinvZ = X.transpose() * RinvZ;
  MatrixXd zSz = ZRinv*Z - xRinvZ.transpose() * xRx_inv.selfadjointView<Lower>() * xRinvZ;
  ...
  Hen.block(0,0,NbCof,NbCof) = xRx;
  Hen.block(0,NbCof,NbCof,NbAlea) = xRinvZ;
  Hen.block(NbCof,0,NbAlea,NbCof) = xRinvZ.transpose();
  ...
  while(((crit>Crit)&&(iteration<MaxIter))||(it==0)){
    ...
    VectorXd yPy = Py.transpose() * R.cwiseProduct(Py);
  ...
  MatrixXd VarBeta = Hen_inv.bottomRightCorner(NbCof,NbCof);
